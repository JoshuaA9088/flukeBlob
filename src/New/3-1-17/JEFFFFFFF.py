...
def learningToPhoto():
    ...
    for x in range(pic.width):
        for y in range(pic.height):
            orderedPair = {}
            rgbStuffs = pic.getRGB(x,y)
            #print(rgbStuffs)
            coloredPixel = colorsys.rgb_to_hsv(rgbStuffs[0],rgbStuffs[1],rgbStuffs[2])
            #coloredPixel = pic.getyuv(x,y)
            #orderedPair.append([x,y])
            
            if coloredPixel[0] >= 110 and coloredPixel[1] >=120 and coloredPixel[2] >=80:
...
def colorTest():
    ...
    for x in range(origPic.width):
        for y in range(origPic.height):
            ...
            #print(rgbStuffs)
            yuvValues=colorsys.rgb_to_hsv(rgbStuffs[0],rgbStuffs[1],rgbStuffs[2])
            #yuvValues=rgb2yuv(rgbStuffs[0],rgbStuffs[1],rgbStuffs[2])
            #print(yuvValues)
            yuvPic.setRGB(x,y,yuvValues[0],0,0)
